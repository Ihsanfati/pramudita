...
export default function SignUp(props) {
  ...
  const [jurusanError, setJurusanError] = React.useState(false);
  const [jurusanErrorMessage, setJurusanErrorMessage] = React.useState('');
  const [stateAsalSekolah, setAsalSekolah] = React.useState('');
  const [stateJurusan, setJurusan] = React.useState('');
  ...
  const validateInputs = () => {
    ...
    // Validasi jurusan
    if (!stateJurusan|| stateJurusan.trim().length === 0) {
      setJurusanError(true);
      setJurusanErrorMessage('Jurusan tidak boleh kosong.');
      isValid = false;
    } else {
      setJurusanError(false);
      setJurusanErrorMessage('');
    }
      ...
      <SignUpContainer direction="column" justifyContent="space-between">
        <Card variant="outlined">
          ...
          >
            ...
            <FormControl fullWidth required error={jurusanError}>
              ...
              <TextField
                ...
                helperText={jurusanErrorMessage}
                color={jurusanError ? 'error' : 'primary'}
              >
