...
def generate_data_ips(jumlah_data):
    ...
    for _ in range(jumlah_data):
        data.append({
            "nama": fake.name(),
            "jurusan": "IPA",
            "biologi": generate_nilai(),
            "fisika": generate_nilai(),
            "kimia": generate_nilai(),
            "matematika": generate_nilai()
        })
    return data

def simpan_ke_xls(data, nama_file):
    workbook = xlwt.Workbook()
    sheet = workbook.add_sheet("Data Siswa IPA")
...
simpan_ke_xls(data_dummy, "data_siswa_ipa.xls")
